import React, { useState, useEffect, useRef } from "react";
import "../utils/quillConfig";
import {
  IonPage,
  IonHeader,
  IonToolbar,
  IonTitle,
  IonContent,
  IonSearchbar,
  IonButton,
  IonIcon,
  IonItem,
  IonLabel,
  IonAccordionGroup,
  IonAccordion,
  IonList,
} from "@ionic/react";
import {
  trash,
  print,
  chevronDownCircleOutline,
  chevronUpCircleOutline,
} from "ionicons/icons";
import { Reorder } from "framer-motion";
import ReactQuill from "react-quill";
import "react-quill/dist/quill.snow.css";
import notesItem from "../utils/notes.json";
import "./document.css";

const modules = {
  toolbar: [
    [{ header: "1" }, { header: "2" }],
    [{ list: "ordered" }, { list: "bullet" }],
    ["bold", "italic", "underline"],
    ["link"],
    ["clean"],
  ],
};

const formats = [
  "header",
  "bold",
  "italic",
  "underline",
  "link",
  "list",
  "divBlock",
  "h2Block",
  "pBlock",
  "ulBlock",
  "liInline",
];
const DocumentGenerator = () => {
  const [notes] = useState(notesItem.notes);
  const [searchQuery, setSearchQuery] = useState("");
  const [sort, setSort] = useState("asc");
  const [sections, setSections] = useState([]);
  const [editorContent, setEditorContent] = useState("");
  const quillRef = useRef(null);

  const handleSort = () => {
    setSort((prev) => (prev === "asc" ? "desc" : "asc"));
  };

  const filteredNotes = notes
    .filter((note) => note.title.toLowerCase().includes(searchQuery))
    .sort((a, b) => {
      if (sort === "asc") {
        return (
          a.category.localeCompare(b.category) || a.title.localeCompare(b.title)
        );
      } else {
        const categoryCompare = b.category.localeCompare(a.category);
        if (categoryCompare !== 0) return categoryCompare;
        return b.title.localeCompare(a.title);
      }
    });

  const groupedNotes = filteredNotes.reduce((groups, note) => {
    if (!groups[note.category]) groups[note.category] = [];
    groups[note.category].push(note);
    return groups;
  }, {});

  const generateHTMLFromContentList = (contentList) => {
    return contentList
      .map((item) => {
        switch (item.type) {
          case "title":
            return `<h2><b>${item.text}</b></h2>`;
          case "header":
            return `<h2>${item.text}</h2>`;
          case "paragraph":
            return `<p>${item.text}</p>`;
          case "list":
            return `<ul>${item.items
              .map((li) => `<li>${li}</li>`)
              .join("")}</ul>`;
          default:
            return "";
        }
      })
      .join("");
  };

  const wrapSection = (title, html) => {
    const wrapper = document.createElement("div");
    wrapper.setAttribute("data-section", title);
    const contentDoc = new DOMParser().parseFromString(html, "text/html");
    Array.from(contentDoc.body.childNodes).forEach((child) => {
      wrapper.appendChild(child);
    });
    return wrapper.outerHTML;
  };

  const handleEditorChange = (value) => {
    // value is in delta format, but you can use editor.root.innerHTML to get raw HTML
    setEditorContent(value); // Set the content to state (raw HTML or delta)
  };

  const handleClick = (note) => {
    if (sections.some((s) => s.title === note.title)) return;
    const html = generateHTMLFromContentList(note.contentList);
    const newSection = { title: note.title, html };
    setSections((prev) => [...prev, newSection]);
  };

  const removeSection = (idx) => {
    setSections((prev) => prev.filter((_, i) => i !== idx));
  };

  const splitEditorContentIntoSections = (content) => {
    const parser = new DOMParser();
    const doc = parser.parseFromString(content, "text/html");
    console.log(doc);

    const nodes = Array.from(doc.body.childNodes);
    console.log(nodes);
    const sections = [];
    let currentSection = null;
    nodes.forEach((node) => {
      if (node.nodeName === "H2") {
        // console.log(first);
        if (currentSection?.title !== null && currentSection?.html !== null) {
          sections.push(currentSection);
        }

        currentSection = {
          title: node.textContent.trim(),
          html: `<h2>${node.textContent.trim()}</h2>`, // will build this up from following nodes
        };
      } else if (currentSection) {
        currentSection.html += node.outerHTML || node.textContent;
      }
      console.log(sections);
    });

    if (currentSection) {
      sections.push(currentSection);
    }

    return sections;
  };

  const handleReorder = (newOrder) => {
    const currentSections = splitEditorContentIntoSections(editorContent);
    console.log(currentSections);

    const reorderedSections = newOrder.map((item) => {
      const match = currentSections.find((s) => {
        return s?.title.includes(item?.title);
      });
      return {
        title: item.title,
        html: match ? match.html : item.html,
      };
    });

    console.log(reorderedSections);
    // Reconstruct editor content
    const newEditorContent = reorderedSections
      .map((section) => `<h2>${section.title}</h2>${section.html}`)
      .join("");
    setEditorContent(newEditorContent);
    setSections(reorderedSections);
  };

  useEffect(() => {
    const combinedHTML = sections
      .map((s) => wrapSection(s.title, s.html))
      .join("");
    setEditorContent(combinedHTML);
  }, [sections]);

  const handlePrint = () => {
    const printWindow = window.open("", "_blank");
    printWindow.document.write(`
      <html>
        <head><title>Guidline for patient</title></head>
        <body>${editorContent}</body>
      </html>
    `);
    printWindow.document.close();
    printWindow.print();
  };

  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle style={{ textAlign: "center" }}>
            Recommendation Guide
          </IonTitle>
        </IonToolbar>
      </IonHeader>

      <IonContent fullscreen>
        <div className="full-layout">
          {/* Left Sidebar */}
          <div className="left-panel">
            <div className="left-half">
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <h4>Notes</h4>
                <IonButton onClick={handleSort}>
                  <IonIcon
                    icon={
                      sort === "asc"
                        ? chevronDownCircleOutline
                        : chevronUpCircleOutline
                    }
                    slot="start"
                  />
                  Sort {sort === "asc" ? "A-Z" : "Z-A"}
                </IonButton>
              </div>
              <IonSearchbar
                debounce={500}
                onIonInput={(e) =>
                  setSearchQuery(e.detail.value?.toLowerCase() || "")
                }
              />
              <IonAccordionGroup>
                {Object.entries(groupedNotes).map(([category, notes], idx) => (
                  <IonAccordion value={category} key={idx}>
                    <IonItem slot="header" color="light">
                      <IonLabel>{category}</IonLabel>
                    </IonItem>
                    <div className="ion-padding" slot="content">
                      <IonList>
                        {notes.map((note, i) => (
                          <IonItem
                            key={i}
                            button
                            onClick={() => handleClick(note)}
                          >
                            <IonLabel>{note.title}</IonLabel>
                          </IonItem>
                        ))}
                      </IonList>
                    </div>
                  </IonAccordion>
                ))}
              </IonAccordionGroup>
            </div>
          </div>

          {/* Right Editor Panel */}
          <div className="right-panel">
            {sections.length !== 0 && (
              <div className="" style={{ width: "50%" }}>
                <h4>Selected Sections</h4>
                <Reorder.Group
                  axis="y"
                  values={sections}
                  onReorder={handleReorder}
                  className="selected-list"
                >
                  {sections.map((section, idx) => (
                    <Reorder.Item
                      key={section.title}
                      value={section}
                      onReorder={handleReorder}
                    >
                      <div className="section-item">
                        <span className="drag-handle">☰</span>
                        <span className="section-title">{section.title}</span>
                        <IonButton
                          size="small"
                          fill="clear"
                          color="danger"
                          onClick={() => removeSection(idx)}
                        >
                          <IonIcon icon={trash} />
                        </IonButton>
                      </div>
                    </Reorder.Item>
                  ))}
                </Reorder.Group>
              </div>
            )}
            <div className="editor">
              <div className="editor-toolbar">
                <h3>Document Preview</h3>
                <IonButton onClick={handlePrint}>
                  <IonIcon icon={print} slot="start" />
                  Print
                </IonButton>
              </div>
              <ReactQuill
                ref={quillRef}
                value={editorContent}
                theme="snow"
                onChange={handleEditorChange}
                formats={formats}
                modules={modules}
                style={{ height: "100%", backgroundColor: "white" }}
              />
            </div>
          </div>
        </div>
      </IonContent>
    </IonPage>
  );
};

export default DocumentGenerator;
